// ===============================
// Element Merge Infinity – FINAL
// Stabilná verzia s core animáciou 2×2
// ===============================
...
function spawnRandomTile() {
  ...
  board[y][x] = { element: randomElement() };
}
...
function renderBoard() {
  ...
  const coreEl = document.getElementById("core-count");
...
// ===============================
// MERGE LOGIC (A – čisté línie)
// ===============================

function handleMerge(x, y) {
  const element = board[y][x].element;

  const horiz = getLine(x, y, element, "h");
  const vert = getLine(x, y, element, "v");
  const square = getSquare(x, y, element);
...
function getLine(x, y, element, dir) {
  const line = [{ x, y }];
  const delta = dir === "h" ? [1, 0] : [0, 1];
...
function getSquare(x, y, element) {
  const out = [];
  const offsets = [
    [0, 0], [-1, 0], [0, -1], [-1, -1]
  ];
  ...
  for (const [ox, oy] of offsets) {
    ...
    if (
      sx >= 0 && sx + 1 < GRID_SIZE &&
      sy >= 0 && sy + 1 < GRID_SIZE &&
      board[sy][sx]?.element === element &&
      board[sy][sx + 1]?.element === element &&
      board[sy + 1][sx]?.element === element &&
      board[sy + 1][sx + 1]?.element === element
    ) {
      out.push(
        { x: sx, y: sy },
        { x: sx + 1, y: sy },
