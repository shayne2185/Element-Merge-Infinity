...
const MAX_BASE_LEVEL = 3; // vyššie levely vznikajú mergovaním
const COMBO_BONUS_THRESHOLD = 5; // 5+ ikon = +2 level
...
function createTileElement(level, x, y) {
  ...
  // Nechceme zobrazovať číslo – iba ikonku
  tile.textContent = "";

  // Zakážeme drag existujúcich tile (dragujeme len z "next" panelu)
...
function generateNextTile() {
  // Jednoduchá logika: väčšinou level 1–3
  const r = Math.random();
  if (r < 0.7) {
...
function renderNextTile() {
  ...
  tile.textContent = ""; // žiadne číslo, len ikonka
  nextTileContainer.appendChild(tile);
}

/* ---------- DRAG NEXT TILE Z PANELU ---------- */
...
nextTileContainer.addEventListener("touchstart", (e) => {
  ...
  window.addEventListener("touchmove", onNextTileDragMoveTouch, { passive: false });
...
// Umiesť next tile na bunku, ak je prázdna
...
function tryMergeAt(x, y) {
  ...
  // odstrániť celú skupinu okrem cieľa
  group.forEach((pos) => {
    if (!(pos.x === x && pos.y === y)) {
      board[pos.y][pos.x] = 0;
    }
...
// nájdi všetkých susedov s rovnakým levelom
