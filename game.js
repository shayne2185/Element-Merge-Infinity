...
function spawnRandomTile() {
  ...
  board[y][x] = { element: ELEMENTS[Math.floor(Math.random() * ELEMENTS.length)] };
}
...
function generateNextTile() {
  nextTile = { element: ELEMENTS[Math.floor(Math.random() * ELEMENTS.length)] };
}
...
function getSquare(x, y, element) {
  ...
  for (const [ox, oy] of offsets) {
    ...
    if (
      ...
      out.push({ x: sx, y: sy }, { x: sx + 1, y: sy }, { x: sx, y: sy +
