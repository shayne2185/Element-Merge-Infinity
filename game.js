  ...
  for (let y = 0; y < GRID_SIZE; y++) {
    for (let x = 0; x < GRID_SIZE; x++) {
      ...
      if (tile) {
        ...
        if (tile.isCore || tile.element === "core") {
          el.style.backgroundImage = 'url("assets/core.png")';
        } else {
...
function renderNextTile() {
  ...
  const el = document.createElement("div");
  el.className = "tile";
  if (nextTile.isCore || nextTile.element === "core") {
    el.style.backgroundImage = 'url("assets/core.png")';
  } else {
  ...
  // neviditeľná vrstva pre drag (kritické pre mobil)
...
function startDrag(x, y) {
  ...
  if (nextTile.isCore || nextTile.element === "core") {
    ghost.style.backgroundImage = 'url("assets/core.png")';
  } else {
...
function finishDrop(x, y) {
  ...
  const placed = board[cy][cx];

  if (placed.isCore || placed.element === "core") {
    explodeCore(cx, cy);
  } else {
    tryMerge(cx, cy);
  }
...
function explodeCore(cx, cy) {
  ...
  affected.forEach(({ x, y }) => {
    ...
    if (cell) {
      ...
      // reflow, aby sa animácia znova spustila
      void cell.offsetWidth;
      cell.classList.add("pulse");
    }
  });

  // po krátkej chvíli vyčistiť
  setTimeout(() => {
    affected.forEach(({ x, y }) => {
      board[y][x] = null;
    });
...
function tryMerge(x, y) {
  const tile = board[y][x];
  if (!tile || tile.isCore || tile.element === "core") return;

  const group = findGroup(x, y, tile.element, tile.level);

  if (group.length < 3) return;

  const makeCore = group.length >= 4;
  ...
  // pokus o ďalší merge na tom istom mieste (chain)
  if (!board[y][x].isCore) {
    tryMerge(x, y);
  }
}
...
function findGroup(x, y, element, level) {
  ...
  while (stack.length) {
    ...
    if (!tile || tile.isCore || tile.element !== element || tile.level !== level)
      continue;
